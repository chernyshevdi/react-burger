...
function ProductConstructorItem({handleClose, moveListItem, index, item, type, isLocked}) {

  const { ingredientsInBurgerConstructor } = useSelector(state => state.constructorReducer);
  const dragDropRef = useRef(null)

 const [, drag] = useDrag({ //добавляю ф-ть перетаскивания
  ...
  const [{opacity}, drop] = useDrop({
    accept: 'item',
    hover: (item) => {
        const dragIndex = ingredientsInBurgerConstructor.other.findIndex(elem => elem._id === item._id)
