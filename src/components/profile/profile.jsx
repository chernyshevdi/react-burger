...
function Profile() {
    ...
    function handleSubmit(e) { //хендел обновления данных
        e.preventDefault();
    ...
    function handleLogout(e) { //хендел выхода из профиля
        e.preventDefault();
    ...
    useEffect(() => { //переход после выхода из профиля
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        if(updatestatus.success === false){//если токен умер 
      ...
      useEffect(() => {
          if(userData.user && name) {
              if(userData.user.name !== name || userData.user.email !== email || password) {
                setInputChange(true)
              }
            else {
                setInputChange(false)
    ...
    return(
        <section className={styleProfile.container}>
                ...
                <nav className={styleProfile.nav}>
                    <ul className={styleProfile.navList}>
                       <Link to='/profile' className={styleProfile.link}>
                            <li 
                                className={`${location.pathname === '/profile' ? styleProfile.list_active : styleProfile.list} text text_type_main-medium text_color_inactive mb-6`}
                            >
                                Профиль
                            </li>
                        </Link>
                        <Link 
                            to='/profile/orders' 
                            className={styleProfile.link}
                        >
                            <li 
                                className={`${location.pathname === '/profile/orders' ? styleProfile.list_active : styleProfile.list} text text_type_main-medium text_color_inactive mb-6`}
                            >
                                История заказов
                            </li>
                        </Link>
            ...
            <div className={styleProfile.footer}>
                <p className={`${styleProfile.info} text text_type_main-default text_color_inactive`}>В этом разделе вы можете изменить свои персональные данные</p>
                
                <div className={inputChange ? styleProfile.buttons_active : styleProfile.buttons}>
